...
def test_carereceiver_update_location_success(client, register_and_link_users):
    ...
    resp = client.post(
        "/user/location", json=location, headers=auth_headers(cr["token"])
    )
...
def test_carereceiver_update_location_no_permission(client, register_user):
    ...
    location = {"latitude": 25.03, "longitude": 121.56}
    resp = client.post("/user/location", json=location, headers=auth_headers(cr_token))
...
def test_caregiver_get_linked_location_success(client, register_and_link_users):
    ...
    location = {"latitude": 25.03, "longitude": 121.56}
    client.post("/user/location", json=location, headers=auth_headers(cr["token"]))
    # caregiver queries
    resp = client.get(
        f"/user/linked-location/{cr['email']}", headers=auth_headers(cg["token"])
    )
    assert resp.status_code == status.HTTP_200_OK
    data = resp.json()
    assert data["latitude"] == location["latitude"]
...
def test_caregiver_get_linked_location_no_link(client, register_user):
    ...
    resp = client.get(
        f"/user/linked-location/{cr_email}", headers=auth_headers(cg_token)
    )
    assert resp.status_code == status.HTTP_403_FORBIDDEN
...
def test_caregiver_get_linked_location_not_found(client, register_and_link_users):
    ...
    resp = client.get(
        "/user/linked-location/notfound@email.com", headers=auth_headers(cg["token"])
    )
...
def test_carereceiver_cannot_get_linked_location(client, register_and_link_users):
    ...
    resp = client.get(
        f"/user/linked-location/{cr['email']}", headers=auth_headers(cr["token"])
    )
    assert resp.status_code == status.HTTP_403_FORBIDDEN
